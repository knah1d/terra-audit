...
# .claude/plans/misty-growing-yao.md. Sent via Resend's HTTPS API, not
...
# doesn't have this problem. RESEND_API_KEY has no safe default (there's
# no dev-only fallback the way JWT_SECRET has a fallback string); instead
# backend/email_util.py falls back to logging the OTP when it's unset, so
...
if not EMAIL_CONFIGURED:
    warnings.warn(
        "RESEND_API_KEY not set — registration OTPs will be logged to "
        "stdout instead of emailed. Set RESEND_API_KEY (and EMAIL_FROM, "
        "verified in your Resend account) in .env before deploying this "
        "anywhere real.",
        stacklevel=2,
    )
